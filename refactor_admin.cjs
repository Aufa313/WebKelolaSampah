...
// 2. Add state for withdrawals and stats
const stateInsertPos = content.indexOf('// --- Pending Deposit States ---');
const newStates = `// --- Stats & Withdrawals States ---
  ...
  `;
content = content.slice(0, stateInsertPos) + newStates + content.slice(stateInsertPos);
...
const renderInsertPos = content.indexOf('{/* 8. RIWAYAT KONSULTASI / CUSTOMER SUPPORT */}');
const withdrawalsRender = `{/* TABEL WITHDRAWAL BARU */}
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-10">
        <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-slate-500 font-sans mt-1">Daftar warga yang mengajukan pencairan uang atau sembako.</p>
          </div>
        </div>
      ...
      `;
content = content.slice(0, renderInsertPos) + withdrawalsRender + content.slice(renderInsertPos);

fs.writeFileSync(path, content, 'utf8');
console.log("DashboardAdmin refactored successfully!");
