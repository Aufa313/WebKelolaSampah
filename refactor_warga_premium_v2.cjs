...
console.log("Restoring original DashboardWarga...");
execSync('git checkout src/components/dashboard/DashboardWarga.tsx');

console.log("Running refactor_warga.cjs first...");
execSync('node refactor_warga.cjs');

console.log("Applying premium features modifications...");
...
content = content.replace(
  ...
  'import { fetchPickups, createPickup, fetchTransactions, fetchLeaderboard, requestWithdrawal, fetchNotifications, markNotificationRead } from "../../services/api";'
);
...
// 2. Add state for notifications and user level logic
// Find where citizenUserId is defined (in the refactored code):
// 'const citizenUserId = parseInt(localStorage.getItem("lengkang_authenticated_user_id") || "3");'
const citizenUserIdStr = '  const citizenUserId = parseInt(localStorage.getItem("lengkang_authenticated_user_id") || "3");';
const premiumStates = `
...
content = content.replace(citizenUserIdStr, citizenUserIdStr + premiumStates);

// 3. Define userLevelInfo after totalWeight state is defined (below const [mutations, setMutations] = useState<Mutation[]>([]);)
const mutationsStateStr = 'const [mutations, setMutations] = useState<Mutation[]>([]);';
content = content.replace(mutationsStateStr, mutationsStateStr + '\n  const userLevelInfo = getWargaLevel(totalWeight);\n');

// 4. Inject Notification Bell in Header (near "Keluar / Log Out")
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-2xl shadow-lg z-50 overflow-hidden font-sans">
                <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                  ...
                  <button onClick={() => setShowNotifications(false)} className="text-slate-405 hover:text-slate-600 text-xs">Tutup</button>
                </div>
...
// 5. Inject Gamifikasi (Progress bar & Level Badge) in Profile Section
const levelBadgeStr = `
...
console.log("DashboardWarga premium features v2 successfully refactored!");
